...
<div class="page-go-url">
    <div class="go-info-wrap">
        ...
        <div class="store-click">
            @if(auth()->check())
                @if(auth()->user()->type == \App\Helper\Static\Vars::PUBLISHER_ROUTE)
                    <a href="{{ route("dashboard", ["type" => \App\Helper\Static\Vars::PUBLISHER_ROUTE]) }}" target="_blank" class="custom-button">Go to Dashboard</a>
                @else
                    <a href="{{ route("dashboard", ["type" => \App\Helper\Static\Vars::ADMIN_ROUTE]) }}" target="_blank" class="custom-button">Go to Dashboard</a>
                @endif
            @endif
        </div>
    </div>
