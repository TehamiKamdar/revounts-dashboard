@php
    $trackingLinkComplete = \App\Models\GenerateLink::where("path", "DeeplinkGenerateJob")->whereMonth("created_at", now()->format("m"))->where("status", 0)->count();
    $trackingLinkPending = \App\Models\GenerateLink::where("path", "DeeplinkGenerateJob")->whereMonth("created_at", now()->format("m"))->where("status", 1)->count();
    $trackingLinkRetry = \App\Models\GenerateLink::where("path", "DeeplinkGenerateJob")->whereMonth("created_at", now()->format("m"))->where("status", 1)->where('is_processing',3)->count();
@endphp
...
<div class="revenue-device-chart">
    ...
    <div class="card broder-0">
        <div class="card-header">
            <h6>Deep Link Current Month Report</h6>
        </div>
        <!-- ends: .card-header -->
        <div class="card-body">
                ...
                <div class="tab-pane fade active show" id="rb_device-today" role="tabpanel" aria-labelledby="rb_device-today-tab">
                    <div class="revenue-pieChart-wrap">
                        <div>
                            ...
                            <canvas id="chartDoughnut4Extra" height="173" style="display: block; width: 426px; height: 173px;" width="426" class="chartjs-render-monitor"></canvas>
                        </div>
                    </div>
...
@push("extended_scripts")
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            chartjsDoughnutExtra("chartDoughnut4Extra", "122", (data = [{{ $trackingLinkComplete }}, {{ $trackingLinkPending }}, {{ $trackingLinkRetry }}]), ["Complete", "Pending", "Failed"]);
        });
    </script>
@endpush
