...
<head>
    <meta charset="UTF-8">
    <title>Please Wait! Link in Process</title>
...
<div class="page-go-url">
    <div class="go-info-wrap">
        ...
        <div class="store-links">The {{ request()->segment(1) == "deeplink" ? "deep" : "tracking" }} link is in process.</div>
        @if(isset($advertiser->name))
            <div class="store-click">{{ request()->segment(1) == "deeplink" ? "Deep" : "Tracking" }} link request receive successfully. Your link will be LIVE within few seconds. Reload this page to verify.<br><br><br>
                <a href="{{ $advertiser->url }}" target="_blank" class="custom-button">Visit {{ $advertiser->name }}</a>
            </div>
        @endif
        <div class="store-tips">
            For any inquiry, please contact <span class="send-btn"><a href="mailto:info@linkscircle.com">info@linkscircle.com</a></span>
        </div>
    </div>
</div>
</body>
