...
class PerformanceService
{
    ...
    use CommissionGraph, CustomCommissionGraph, TransactionGraph, CustomTransactionGraph, PerformanceGraph, CustomPerformanceGraph, SalesGraph, CustomSalesGraph, ClickGraph, CustomClickGraph;
    ...
    public function list(Request $request)
    {
        ...
        if(auth()->user()->status == "active" && $websites) {
            ...
            $conversionPerformanceList = $this->getConversionPerformanceList($request);

        }

        $performanceOverview = $performanceOverview ?? [];
        $performanceOverviewList = $earningPerformanceList['earning'] ?? [];
        $performanceOverviewList2 = $conversionPerformanceList['conversion'] ?? [];
        ...
        if($request->ajax())
        {
            $view = view("template.publisher.reports.performance.list_view", compact('performanceOverviewList'))->render();
            $view2 = view("template.publisher.reports.performance.list_view_2", compact('performanceOverviewList2'))->render();
            return response()->json(['total' => $total, 'total2' => $total2, 'html' => $view, 'html2' => $view2, 'performanceOverview' => $performanceOverview]);
        }

        return view("template.publisher.reports.performance.list", compact('performanceOverview','performanceOverviewList', 'performanceOverviewList2', 'total', 'total2'));
