...
namespace App\Service\Admin\Statistics\TrackingLink;
...
class IndexService
{
    ...
    public function init(Request $request)
    {
        if($request->ajax())
        {
            ...
            // VIEW
            $viewGate      = 'crm_statistics_links_show';

            // DELETE
            $deleteGate    = 'crm_statistics_links_delete';
        ...
        SEOMeta::setTitle(trans('link.statistics.links.title') . " " . trans('global.list'));

        return view('template.admin.statistics.links.index');
    ...
    private function query(Request $request): Collection
    {
        ...
        // DEFINE
        $customQuery = Tracking::select(["trackings.id", "trackings.advertiser_id", "advertisers.name as advertiser_name", "publisher_id", "users.first_name as first_name", "users.last_name as last_name", "website_id", "websites.name as website_name", "trackings.updated_at", "trackings.hits", "trackings.unique_visitor"]);

        $customQuery = $customQuery
                            ->join("advertisers", "advertisers.id", "=", "trackings.advertiser_id")
                            ->join("users", "users.id", "=", "trackings.publisher_id")
                            ->join("websites", "websites.id", "=", "trackings.website_id");
        ...
        if(!empty($search))
        {
            ...
            $totalFilteredQuery = $customQuery->where(function($query) use ($search) {
                $query
                    ...
                    ->orWhere('trackings.updated_at', 'like', "%{$search}%");
            });
