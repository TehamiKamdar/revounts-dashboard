...
{
    use Action;
    public function init(Request $request, Tracking $link)
    {
        if($request->ajax())
        {
            // EDIT
            ...
            return $this->showGridPrepareListing($request, $link);
        }

        SEOMeta::setTitle(trans('global.show') . " " . trans('advertiser.api-advertiser.title_singular'));

        $this->loadAdvertiser($link);
        $this->loadPublisher($link);
        $this->loadWebsite($link);

        return view('template.admin.statistics.links.show', compact('link'));
    ...
    private function showGridPrepareListing(Request $request, Tracking $link)
    {

        $queryData = $this->showGridQuery($request, $link);
    ...
    private function showGridQuery(Request $request, Tracking $link): Collection
    {
        // COLUMN NAMES
        $columns = array(
            "ip_address",
        ...
        // DEFINE
        $customQuery = TrackingDetail::select("*")->where("tracking_id", $link->id);
